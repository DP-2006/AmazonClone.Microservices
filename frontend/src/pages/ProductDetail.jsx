...
import SimilarProductsReviews from '../components/SimilarProductsReviews';
...
export default function ProductDetail() {
  ...
  useEffect(() => {
    ...
    reviewsApi.summary(id).then(r => setRating(r.data)).catch(() => {});
  }, [id]);
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: 40 }}>
      <div style={{ display: 'flex', gap: 40, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: '1 1 400px', background: 'white', padding: 30, borderRadius: 8 }}>
          <Link to="/" style={{ fontSize: 13 }}>← بازگشت</Link>
          <h1 style={{ marginTop: 12, marginBottom: 8 }}>{product.name}</h1>

          {rating && rating.totalCount > 0 && (
            <StarRating value={rating.averageRating} count={rating.totalCount} />
          )}

          <p style={{ color: '#666', lineHeight: 1.6, margin: '12px 0 20px' }}>{product.description}</p>
          ...
          <div style={{ marginBottom: 20 }}>
            <label style={{ marginRight: 8 }}>تعداد:</label>
      ...
      {/* فیچر ۵: نظرات محصولات مشابه */}
      <SimilarProductsReviews categoryId={product.categoryId} excludeProductId={id} />
    </div>
  );
}
